...
def main():
	st.write("""
		## Ayurvedic Leaf Image Detection on Raspberry Pi 
	...
	if (st.button("Predict")):
		if image_file is not None:
			...
			with c2:
				...
				result = ""
				model = keras.models.load_model("D:\dataset\pi_running\mod\CNN_model")
				...
				if(acc<0.5):
					...
					st.write(labels[result])
					res_img = labels[result]+".tif"
					st.image(res_img)
				else:
					st.success("This leaf is from the species: {} => Accuracy: {}%".format(labels[result],round(acc*100,2)))
					st.write(labels[result])
					res_img = labels[result]+".tif"
					#st.image(res_img, caption= "Matched Image", width=100)
			...
			st.image(res_img, caption="Matched Image", width=100)
				#path="D:\dataset\pi_running\image"

				#st.image(search, width=100)
		else:
